...
const assignStaffToPlaceholders = (projects: ProjectInput[], config: GlobalConfig, teamFilter: string = 'All Teams') => {
    ...
    interface Task {
        ...
        team: string;
        targetRole: string; // The role we are trying to fill
    }

    const tasks: Task[] = [];

    workingProjects.forEach((p: ProjectInput) => {
        // Only attempt to fill placeholders for projects that match the current team filter
    ...
    tasks.forEach(task => {
        ...
        const candidates = config.staffTypes.filter(s => 
            ...
            (task.targetRole ? s.role === task.targetRole : true) // Match role if template specified it
        );
...
const optimizeProjectTiming = (
  ...
  teamFilter: string = 'All Teams'
    ...
    // Only allow moving projects that match the team filter and are not locked
    ...
    bestProjects.forEach((p, idx) => {
        // Apply final sanity check only to unlocked projects that match filter
        const matchesFilter = teamFilter === 'All Teams' || p.team === teamFilter;
